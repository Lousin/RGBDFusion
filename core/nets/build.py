...
Channels =[64, 88, 112, 160, 224, 288, 384]
...
class SPNv2(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.channel = Channels[cfg.MODEL.EFFICIENTDET_PHI]
        # Build backbone
        self.imgbackbone   = _build_backbone(cfg)
        self.depthbackbone = _build_backbone(cfg)
        self.MLP = MLP(self.channel*2, self.channel, self.channel)
    ...
    def forward(self, x, is_train=False, gpu=torch.device('cpu'), **targets):
        ...
        images, depth = x
        images_feature = self.backbone(images.to(gpu, non_blocking=True))
        depth_feature = self.backbone(depth.to(gpu, non_blocking=True))
        assert images_feature.shape == depth_feature.shape
        x = torch.cat([images_feature, depth_feature], dim=1)
