...
from core.utils.postprocess import quat2dcm
logger = logging.getLogger(__name__)

class RAPTORDataset(torch.utils.data.Dataset):
    """ PyTorch Dataset class for RAPTORDataset
    ...
    def __init__(self,
        ...
        self.depthfolder = 'images_768x512_depth'
        self.maskfolder  = 'masks_192x128'
        self.stylefolder = 'styles_768x512_RGB'

        # Load CSV & determine image domain
        self.csv = self._load_csv(cfg, split)
        self.domain = cfg.DATASET.DOMAIN
    ...
    def _load_image(self, index, folder=None):
        ...
        # Read
        img_name = self.csv.iloc[index, 0]  # 获取csv中的图片名
        img_file_name = f"scene{img_name}.png"  # 构建实际的文件名
        imgpath = join(self.root, 'camera_output', img_file_name)
        imagedata = cv2.imread(imgpath, cv2.IMREAD_COLOR)
        return imagedata

    def _load_depth(self, index, folder=None):
        """ Read image of given index from a folder, if specified """

        # Overwrite image folder if not provided
        if folder is None:
            folder = self.depthfolder
        # Read
        img_name = self.csv.iloc[index, 0]  # 获取csv中的图片名
        depth_file_name = f"distances{img_name}.png"
        depthpath = join(self.root, 'distances', depth_file_name)
        depthdata = cv2.imread(depthpath, cv2.IMREAD_GRAYSCALE)
        return depthdata

    def _load_mask(self, index):
        """ Read mask image """
        mask_name = self.csv.iloc[index, 0]
        mask_file_name = f"masks{mask_name}.png"
        imgpath = join(self.root, self.domain, self.maskfolder, mask_file_name)
    ...
    def _load_annotations(self, index):
        ...
        bbox /= np.array([self.image_size[0], self.image_size[1], self.image_size[0], self.image_size[1]],
                         dtype=np.float32)
        ...
        keypts_x = np.array(self.csv.iloc[index, 5:31], dtype=np.float32)
        keypts_y = np.array(self.csv.iloc[index, 31:57], dtype=np.float32)
        keypts = np.concatenate((keypts_x, keypts_y))
        keypts = np.reshape(keypts, (self.num_keypts, 2))  # (N, 2)

        # Ground-truth pose
        q_gt = np.array(self.csv.iloc[index, 61:65], dtype=np.float32)  # [qw, qx, qy, qz]
        t_gt = np.array(self.csv.iloc[index, 58:61], dtype=np.float32)

        annotations = {'imgpath': f"scene{self.csv.iloc[index, 0]}.png",  # Image file name, e.g., 'img000001.jpg'
